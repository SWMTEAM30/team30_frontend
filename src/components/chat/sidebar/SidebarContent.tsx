...
export default function SidebarContent({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={cn('flex flex-col space-y-4 p-4 items-start h-full overflow-y-auto scrollbar-hide', className)}>
      <button
      ...
      >
        ...
        {isSidebarOpen && <span className="ml-3 font-medium whitespace-nowrap">새 채팅</span>}
      </button>
      <div className="flex-1 flex flex-col gap-2 w-full">
        {chatRooms.map((room, i) => (
          <button
            key={i}
            className={`flex items-center w-full px-4 h-12 rounded-lg transition-all duration-200 ${'hover:bg-yellow-200 text-blue'}`}
          >
            <MessageSquare className="w-5 h-5" />
            {isSidebarOpen && <span className="ml-3 font-medium whitespace-nowrap">{room.title}</span>}
          </button>
        ))}
      </div>
      <button className="flex items-center w-full px-4 h-12 rounded-lg text-gray-600 hover:text-gray-800 hover:bg-gray-100 mt-auto mb-2">
        ...
        {isSidebarOpen && <span className="ml-3 font-medium whitespace-nowrap">설정</span>}
      </button>
    </div>
  );
}
