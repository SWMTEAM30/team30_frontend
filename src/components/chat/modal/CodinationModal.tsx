    ...
    >
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full md:w-4/5 max-w-4xl max-h-[95vh] flex flex-col">
        ...
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">코디하기</h2>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">옷장에서 선택하세요</h3>
          {closet.length === 0 ? (
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              <p>옷장이 비어있습니다</p>
              <p className="text-sm mt-1">AI와 대화하여 패션 아이템을 옷장에 추가해보세요</p>
            </div>
          ) : (
              ...
              {closet.map((cloth) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* 정보 */}
                    <div className="p-2">
                      <h4 className="font-medium text-gray-900 dark:text-white text-sm truncate">{cloth.name}</h4>
                    </div>
                  </div>
                );
              })}
        ...
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 mt-4">
          <button
            onClick={onClose}
            className="w-full sm:flex-1 px-4 py-3 text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-600 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors"
          >
            취소
          </button>
          <button
            ...
            className="w-full sm:flex-1 px-4 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
          >
