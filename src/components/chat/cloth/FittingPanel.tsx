...
// import PanelExtensionButton from '@/components/chat/panel/PanelExtensionButton';
...
// function ImageTabButton({ tab, isActive }: { tab: any; isActive: boolean }) {
//   const setActiveImageTabId = useSetAtom(activeImageTabIdAtom);
...
//       className={`${isActive ? 'ring-2 ring-blue-500' : 'hover:opacity-80'}`}
//     >
...
// function ImageDetailPanel({ imageData }: { imageData: MessageImage | null }) {
...
// export default function FittingPanel({ top }: { top: number }) {
...
//         renderTab={(tab, isActive) => <ImageTabButton tab={tab} isActive={isActive} />}
//         renderContent={(activeTab) => <ImageDetailPanel imageData={activeTab} />}
